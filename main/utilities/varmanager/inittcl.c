    ...
    char **argv)		/* Values of command-line arguments. */
{
    if (argc < 2) {
        ...
        fprintf(stderr,"  inittcl script ?params...?\n");
        exit(-1);
    }
    /*
...
#define TCL_LOCAL_APPINIT Tcl_AppInit
    ...
    cmdArgc = argc-1;          /* Strip off the command name... */
    cmdArgv = &(argv[1]);

    Tcl_Main(1, argv, TCL_LOCAL_APPINIT);
    ...
    Tcl_Interp *interp)		/* Interpreter for application. */
{
    ...
    /*
     * extract the script name and set the remaining parameters in argv
     */
    script = *cmdArgv++; cmdArgc--;
    arglist= Tcl_NewListObj(0, NULL);
    while (cmdArgc > 0) {
        element = Tcl_NewStringObj(*cmdArgv++, -1);
        Tcl_ListObjAppendElement(interp, arglist, element);
        cmdArgc--;
    }
    Tcl_SetVar(interp, "argv", Tcl_GetString(arglist), TCL_LEAVE_ERR_MSG | TCL_GLOBAL_ONLY );
