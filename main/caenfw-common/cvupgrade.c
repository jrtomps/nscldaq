...
static void Usage() {
		...
		printf("-cfa: specifies that the config file to be updated is in CFA format.\n");
		printf("      With CFA files the -param option is not required.\n");
...
int cvUpgrade(cvUpgradeParams *Config, cvFlashAccess *Flash)
{
	...
	if (!Config->cfa) {
		...
		if (validateModel(Config, Flash, (char**)&CFdata, &CFsize)) {
...
int main(int argc,char *argv[])
{
	...
	*Params.modelName= '\0';
	...
	if (strcmp(argv[argIndex],"-KeyInfo") == 0 ) {
	  ...
	  sprintf(Params.ConfFile, "%s",  argv[argIndex]);
	}
	argIndex++;
