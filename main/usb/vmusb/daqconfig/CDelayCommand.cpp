...
*/
CDelayCommand::CDelayCommand(CTCLInterpreter&   interp,
...
*/
int
CDelayCommand::operator()(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
...
/*
  ...
  Side effects:
     ...
     - On success, this is the name of the module. allowing e.g.
       delay config [delay create mydelay value ] ....
*/
int
CDelayCommand::create(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  ...
  if ((value == 0) && (errno != 0)) {
    Usage("Invalid value for delay value.", objv);
    return TCL_ERROR;
  }
...
*/
int
CDelayCommand::config(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  ...
  CReadoutModule* pModule = m_Config.findAdc(name);
  if (!pModule) {
    Usage("Delay module does not exist", objv);
    return TCL_ERROR;
  }
...
*/
int
CDelayCommand::cget(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  if (objv.size() != 3) {
...
void
CDelayCommand::Usage(std::string msg, std::vector<CTCLObject>& objv)
{
  string result("ERROR: ");
  result += msg;
  result += "\n";
  ...
  result += "\n";
  result += "Usage\n";
  result += "    delay create name value\n";
  result += "    delay config name config-params...\n";
  result += "    delay cget name";
