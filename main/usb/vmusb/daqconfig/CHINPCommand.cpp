...
*/
int
CHINPCommand::operator()(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
...
CHINPCommand::create(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  ...
  CHINP* pHINP = new CHINP;
  pModule    = new CReadoutModule(name, *pHINP);
...
CHINPCommand::config(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  ...
  CReadoutModule* pModule = m_Config.findAdc(name);
  if (!pModule) {
    Usage("XLM module does not exist", objv);
    return TCL_ERROR;
  }
...
void
CHINPCommand::Usage(std::string msg, std::vector<CTCLObject>& objv)
{
  string result("ERROR: ");
  result += msg;
  result += "\n";
  ...
  result += "\n";
  result += "Usage\n";
  result += "    hinp create name base-address\n";
  result += "    hinp config name config-params...\n";
  result += "    hinp cget name";
