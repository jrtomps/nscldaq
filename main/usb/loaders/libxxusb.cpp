...
// 06/20/08	CAMAC Read and Write bug fixed
// 08/24/2009	CAMACDGG: improved and added firmware version detection for proper setting of outputs with FW 5.01
	                            ...
	                            int delay, int gate, short invert, short latch)
{
   long Data, SL_Data;
	...
	long Delay_ext;
// Added 08/209 firmware version detection for proper setting of outputs with FW 5.01
...
// CAMAC direct write function
    ret = -1;  // added this initialization to indicate an error on condition following if-block not executed NMG.
    intbuf[0]=1;
    intbuf[1]=(long)(F+A*32+N*512 + 0x4000);
    if ((F > 15) && (F < 24))
    {	
