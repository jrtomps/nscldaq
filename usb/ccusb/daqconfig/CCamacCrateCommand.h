...
#ifndef __CCAMACCRATECOMMAND_H
#define __CCAMACCRATECOMMAND_H
...
class CCamacCrateCommand : public CTCLObjectProcessor
{
private:
  CConfiguration& m_Config;

public:
  CCamacCrateCommand(CTCLInterpreter&     interp,
		 CConfiguration&      config,
		 std::string          commandName = std::string("CamacCrate"));
  virtual ~CCamacCrateCommand();

private:
  CCamacCrateCommand(const CCamacCrateCommand& rhs);
  CCamacCrateCommand& operator=(const CCamacCrateCommand& rhs);
  int operator==(const CCamacCrateCommand& rhs) const;
  int operator!=(const CCamacCrateCommand& rhs) const;
public:
