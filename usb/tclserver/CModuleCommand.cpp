		       ...
		       vector<CTCLObject>& objv)
{
  ...
  CControlModule* pModule = m_Server.createModule(type);
  if (!pModule) {
    string result = "No such module type: ";
    result       += type;
    interp.setResult(result);
    return TCL_ERROR;
  }

  // Create the configuration for the module and attach it:

  CItemConfiguration* pConfiguration = new CItemConfiguration(name);
  pModule->Attach(pConfiguration);
  m_Server.addModule(pModule);
