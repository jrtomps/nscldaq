...
*/
CVMECamacBranchCommand::CVMECamacBranchCommand(CTCLInterpreter&   interp,
...
/*!
   Destructor:
*/
CVMECamacBranchCommand::~CVMECamacBranchCommand()
{
}
//////////////////////////////////////////////////////////////////////
//////////////// Command processing //////////////////////////////////
...
*/
int
CVMECamacBranchCommand::operator()(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
...
/*
  ...
  Side effects:
     ...
     - On success, this is the name of the module. allowing e.g.
       VMECamacBranch config [VMECamacBranch create br0 0 ] ....
*/
int
CVMECamacBranchCommand::create(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  ...
  if ((value == 0) && (errno != 0)) {
    Usage("Invalid value for branch id value.", objv);
    return TCL_ERROR;
  }
  ...
  CVMECamacBranch* pDelay = new CVMECamacBranch;
  pModule    = new CReadoutModule(name, *pDelay);
  pModule->configure("-branch", sValue);
...
*/
int
CVMECamacBranchCommand::config(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  ...
  CReadoutModule* pModule = m_Config.findAdc(name);
  if (!pModule) {
    Usage("VMECamacBranch module does not exist", objv);
    return TCL_ERROR;
  }
...
*/
int
CVMECamacBranchCommand::cget(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  if (objv.size() != 3) {
...
void
CVMECamacBranchCommand::Usage(std::string msg, std::vector<CTCLObject>& objv)
{
  string result("ERROR: ");
  result += msg;
  result += "\n";
  ...
  result += "\n";
  result += "Usage\n";
  result += "    VMECamacBranch create name value\n";
  result += "    VMECamacBranch config name config-params...\n";
  result += "    VMECamacBranch cget name";
