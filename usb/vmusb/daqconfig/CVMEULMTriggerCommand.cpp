...
*/
CVMEULMTriggerCommand::CVMEULMTriggerCommand(CTCLInterpreter& interp,
...
/*!
   ...
   etc.
*/
CVMEULMTriggerCommand::~CVMEULMTriggerCommand()
{
}
...
*/
int
CVMEULMTriggerCommand::operator()(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
...
/*
   ...
   - ensure we have a valid ulm name, and slot number 
  ...
  Side effects:
     ...
     - On success, this is the name of the module. allowing e.g.
       VMEULMTrigger config [VMEULMTrigger create stack1] ....
*/
int
CVMEULMTriggerCommand::create(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  // Need to have exactly 4 elements, command 'create' name '

  if (objv.size() != 4) {
    Usage("Expected format \"VMEULMTrigger create name slot_index\" not provided", objv);
    return TCL_ERROR;
  }
  ...
  CULMTrigger<CCBD8210CrateController,CCBD8210ReadoutList> ulm;
  CReadoutHardware *pHardware = compat_clone(ulm);
...
*/
int
CVMEULMTriggerCommand::config(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  ...
  CReadoutModule* pModule = m_Config.findAdc(name);
  if (!pModule) {
    Usage("VMEULMTrigger module does not exist", objv);
    return TCL_ERROR;
  }
...
*/
int
CVMEULMTriggerCommand::cget(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  if (objv.size() != 3) {
...
*/
CConfiguration* 
CVMEULMTriggerCommand::getConfiguration()
{
  return &m_Config;
}
...
void
CVMEULMTriggerCommand::Usage(std::string msg, std::vector<CTCLObject>& objv)
{
  string result("ERROR: ");
  result += msg;
  result += "\n";
  ...
  result += "\n";
  result += "Usage\n";
  result += "    VMEULMTrigger create name\n";
  result += "    VMEULMTrigger config name config-params...\n";
  result += "    VMEULMTrigger cget name";
