...
/*!
   ...
   \param config : CConfiguration& config
       The configuration that will be manipulated by this command.
   \param commandName std::string
       Name of the command to register.
*/
CVMECamacCrateCommand::CVMECamacCrateCommand(CTCLInterpreter&   interp,
...
/*!
   Destructor:
*/
CVMECamacCrateCommand::~CVMECamacCrateCommand()
{
}
//////////////////////////////////////////////////////////////////////
//////////////// Command processing //////////////////////////////////
...
*/
int
CVMECamacCrateCommand::operator()(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
...
/*
   ...
   - ensure we have a valid name, and base address.
  ...
  Side effects:
     ...
     - On success, this is the name of the module. allowing e.g.
       VMECamacCrate config [VMECamacCrate create br0 0 ] ....
*/
int
CVMECamacCrateCommand::create(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  // Need to have exactly 4 elements, command 'create' name base.

  if (objv.size() != 4) {
    Usage("Improper number of parameters for create subcommand. Expecting only four", objv);
    return TCL_ERROR;
  }
  ...
  if ((value == 0) && (errno != 0)) {
    Usage("Invalid value for crate id value.", objv);
    return TCL_ERROR;
  }
  ...
  CCamacCrate<CVMUSB,CVMUSBReadoutList> hybrid_module;
  CReadoutHardware* hardware = compat_clone(hybrid_module);
  pModule    = new CReadoutModule(name, *hardware);
  // clean up after ourselves
  delete hardware;
  pModule->configure("-crate", sValue);
...
*/
int
CVMECamacCrateCommand::config(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  ...
  CReadoutModule* pModule = m_Config.findAdc(name);
  if (!pModule) {
    Usage("VMECamacCrate module does not exist", objv);
    return TCL_ERROR;
  }
...
*/
int
CVMECamacCrateCommand::cget(CTCLInterpreter& interp, vector<CTCLObject>& objv)
{
  if (objv.size() != 3) {
...
void
CVMECamacCrateCommand::Usage(std::string msg, std::vector<CTCLObject>& objv)
{
  string result("ERROR: ");
  result += msg;
  result += "\n";
  ...
  result += "\n";
  result += "Usage\n";
  result += "    VMECamacCrate create name crate_index\n";
  result += "    VMECamacCrate config name config-params...\n";
  result += "    VMECamacCrate cget name";
