...
#ifndef __CDELAYCOMMAND_H
#define __CDELAYCOMMAND_H
...
class CDelayCommand : public CTCLObjectProcessor
{
private:
  CConfiguration& m_Config;

public:
  CDelayCommand(CTCLInterpreter&     interp,
		 CConfiguration&      config,
		 std::string          commandName = std::string("delay"));
  virtual ~CDelayCommand();

private:
  CDelayCommand(const CDelayCommand& rhs);
  CDelayCommand& operator=(const CDelayCommand& rhs);
  int operator==(const CDelayCommand& rhs) const;
  int operator!=(const CDelayCommand& rhs) const;
public:
