...
#ifndef __CVMECAMACCRATECOMMAND_H
#define __CVMECAMACCRATECOMMAND_H
...
class CVMECamacCrateCommand : public CTCLObjectProcessor
{
private:
  CConfiguration& m_Config;

public:
  CVMECamacCrateCommand(CTCLInterpreter&     interp,
		 CConfiguration&      config,
		 std::string          commandName = std::string("VMECamacCrate"));
  virtual ~CVMECamacCrateCommand();

private:
  CVMECamacCrateCommand(const CVMECamacCrateCommand& rhs);
  CVMECamacCrateCommand& operator=(const CVMECamacCrateCommand& rhs);
  int operator==(const CVMECamacCrateCommand& rhs) const;
  int operator!=(const CVMECamacCrateCommand& rhs) const;
public:
