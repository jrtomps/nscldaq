...
CGDG::operator=(const CGDG& rhs) 
{
  if (this != &rhs) {
    for (int i =0; i < 8; i++) {
      m_delays[i] = rhs.m_delays[i];
      m_widths[i] = rhs.m_widths[i];
    }
  }
  return *this;
}
...
{
  return CVMUSBControlModule::operator==(rhs);
}
int
CGDG::operator!=(const CGDG& rhs) const
{
...
CGDG::onAttach()
{
 ...
 m_pConfiguration->addParameter("-base", CItemConfiguration::isInteger, NULL, 
			     string("0"));
...
/*!
    Clone ourselves... Virtual copy constructor essentially.
*/
CControlModule*
CGDG::clone()
{
