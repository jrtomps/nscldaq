...
////////////////////////////////////////////////////////////////////
/*!
  Given a usb_device, return its serial number string ("" if no
  serial number is defined).
  \param aDevice - pointer to the usb_device struct that represents the
                   device in the USB data structures.
  \return string
  \retval "" - device has no serial number.
  \retval other - the stringified serial number (serial numbers need not be integers).

*/
string
CVMUSB::getSerial(struct usb_device* aDevice)
{
  int stringIndex = aDevice->descriptor.iSerialNumber;
  if (stringIndex == 0) {
    return string("");
  }

  usb_dev_handle* handle = usb_open(aDevice);
  char serialNumber[1024];
  int nchars = usb_get_string_simple(handle, stringIndex, serialNumber, sizeof(serialNumber));
  serialNumber[nchars] = 0;
  usb_close(handle);
