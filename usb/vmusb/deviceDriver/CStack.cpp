...
static CItemConfiguration::limit iplLow(1);
...
static CItemConfiguration::limit stackLow(2);
static CItemConfiguration::limit stackHigh(7);
static CItemConfiguration::Limits stackRange(stackLow, stackHigh);

// -vector is in the range 0x0 to 0x7f as there are only 8 bits...and I have to
//         mulitiply it by 2.

static CItemConfiguration::limit vectorLow(0);
static CItemConfiguration::limit vectorHigh(0x7f);
...
//
static CItemConfiguration::limit delayLow(0);
static CItemConfiguration::limit delayHigh(0xff);
static CItemConfiguration::Limits delayRange(delayLow, delayHigh);
...
CStack::onAttach()
{
  ...
  m_pConfiguration->addParameter("-trigger", 
				 CItemConfiguration::isEnum, &validTriggers,
				 "nim1");
  ...
  m_pConfiguration->addParameter("-period",
				 CItemConfiguration::isInteger, NULL, "2");
  m_pConfiguration->addParameter("-stack",
				 CItemConfiguration::isInteger, &stackRange, "2");
  m_pConfiguration->addParameter("-delay",
				 CItemConfiguration::isInteger, &delayRange, "0");
  m_pConfiguration->addParameter("-vector",
				 CItemConfiguration::isInteger, &vectorRange);
  m_pConfiguration->addParameter("-ipl",
				 CItemConfiguration::isInteger, &iplRange, "6");
...
CStack::getStackElements()
{
  ...
  for (int i=0; i < argc; i++) {
    string name(argv[i]);
    CConfiguration::ConfigurationIterator p = pConfiguration->findObjectByName(name);
    CConfigurableObject* pObj = p->s_pObject;
    result.push_back(reinterpret_cast<CVMUSBConfigurableObject*>(pObj));
  }
...
CStack::moduleChecker(string name, string proposedValue, void* arg)
{
  ...
  int status = Tcl_SplitList(NULL, proposedValue.c_str(), &argc, &argv);
  if (status != TCL_OK) {
    return false;
  }
