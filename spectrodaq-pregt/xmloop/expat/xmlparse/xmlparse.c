...
{
  XML_Parser parser = MALLOC("XML_ParserCreate[1]",sizeof(Parser));
  if (!parser)
    return parser;
  ...
  atts = (ATTRIBUTE *)MALLOC("XML_ParserCreate[2]",attsSize * sizeof(ATTRIBUTE));
  nSpecifiedAtts = 0;
  dataBuf = (XML_Char *)MALLOC("XML_ParserCreate[3]",INIT_DATA_BUF_SIZE * sizeof(XML_Char));
  groupSize = 0;
  groupConnector = 0;
...
void destroyBindings(BINDING *bindings)
{
  for (;;) {
    ...
    FREE("destroyBindings[1]",b->uri);
    FREE("destroyBindings[2]",b);
  }
}
...
void XML_ParserFree(XML_Parser parser)
{
  for (;;) {
    ...
    FREE("XML_ParserFree[1]",p->buf);
    destroyBindings(p->bindings);
    FREE("XML_ParserFree[2]",p);
  }
...
#endif /* XML_DTD */
  dtdDestroy(&dtd);
  FREE("XML_ParserFree[3]",(void *)atts);
  FREE("XML_ParserFree[4]",groupConnector);
  FREE("XML_ParserFree[5]",buffer);
  FREE("XML_ParserFree[6]",dataBuf);
  FREE("XML_ParserFree[7]",unknownEncodingMem);
  if (unknownEncodingRelease)
    unknownEncodingRelease(unknownEncodingData);
  FREE("XML_ParserFree[8]",parser);
}
...
int XML_Parse(XML_Parser parser, const char *s, int len, int isFinal)
{
  if (len == 0) {
	...
	buffer = buffer == 0 ? (char *)MALLOC("XML_Parser[1]",len * 2) : (char *)REALLOC("XML_Parse[1]",buffer, len * 2);
...
void *XML_GetBuffer(XML_Parser parser, int len)
{
  if (len > bufferLim - bufferEnd) {
	...
	bufferSize *= 2;
      ...
      newBuf = (char *)MALLOC("XML_GetBuffer",bufferSize);
      if (newBuf == 0) {
	errorCode = XML_ERROR_NO_MEMORY;
	return 0;
      }
  ...
  for (;;) {
	...
	*nextPtr = s;
	...
	if (freeTagList) {
	  ...
	  tag = (TAG *)MALLOC("doContent[1]",sizeof(TAG));
	  if (!tag)
	    return XML_ERROR_NO_MEMORY;
	  tag->buf = (char *)MALLOC("doContent[2]",INIT_TAG_BUF_SIZE);
	  if (!tag->buf)
	    return XML_ERROR_NO_MEMORY;
	...
	if (nextPtr) {
	  ...
	  if (tag->rawNameLength + (int)(sizeof(XML_Char) - 1) + (int)sizeof(XML_Char) > tag->bufEnd - tag->buf) {
	    ...
	    tag->buf = (char *)REALLOC("doContent[1]",tag->buf, bufSize);
	    if (!tag->buf)
	      return XML_ERROR_NO_MEMORY;
	...
	if (startElementHandler) {
	  ...
	  for (;;) {
	    ...
	    tag->buf = (char *)REALLOC("doContent[2]",tag->buf, bufSize);
	    if (!tag->buf)
	      return XML_ERROR_NO_MEMORY;
	    tag->bufEnd = tag->buf + bufSize;
	    if (nextPtr)
	...
	if (endElementHandler) {
	  ...
	  endElementHandler(handlerArg, name.str);
        ...
        if (name.expandStr) {
          ...
          FREE("doContent[1]",name.expandStr); 
          name.expandStr = 0;
          name.expandLen = 0;
        }
	poolClear(&tempPool);
	...
	if (endElementHandler && tag->name.str) {
	  ...
	  reportDefault(parser, enc, s, next);
        ...
        if (tag->name.expandStr) {
          ...
          FREE("doContent[1]",tag->name.expandStr); 
          tag->name.expandStr = 0;
          tag->name.expandLen = 0;
        }
				...
				BINDING **bindingsPtr)
{
  ...
  if (n + nDefaultAtts > attsSize) {
    ...
    atts = (ATTRIBUTE *)REALLOC("storeAtts[1]",(void *)atts, attsSize * sizeof(ATTRIBUTE));
    if (!atts)
      return XML_ERROR_NO_MEMORY;
    if (n > oldAttsSize)
  ...
  if (tagNamePtr->expandLen < binding->uriAlloc) {
    if (tagNamePtr->expandLen <= 0) {
      tagNamePtr->expandStr = (XML_Char *)MALLOC("storeAtts[1]",binding->uriAlloc * 2 * sizeof(XML_Char));
    } else {
      tagNamePtr->expandStr = (XML_Char *)REALLOC("storeAtts[2]",tagNamePtr->expandStr,binding->uriAlloc * 2 * sizeof(XML_Char));
    }
  ...
  do {
    if (i == tagNamePtr->expandLen) {
      tagNamePtr->expandStr = (XML_Char *)REALLOC("storeAtts[3]",tagNamePtr->expandStr, (tagNamePtr->expandLen *= 2) * sizeof(XML_Char));
      if (!tagNamePtr->expandStr)
	return XML_ERROR_NO_MEMORY;
    }
...
int addBinding(XML_Parser parser, PREFIX *prefix, const ATTRIBUTE_ID *attId, const XML_Char *uri, BINDING **bindingsPtr)
{
  ...
  if (freeBindingList) {
    b = freeBindingList;
    if (len > b->uriAlloc) {
      b->uri = (XML_Char *)REALLOC("addBinding[1]",b->uri, sizeof(XML_Char) * (len + EXPAND_SPARE));
      if (!b->uri)
	return 0;
    ...
    b = (BINDING *)MALLOC("addBinding[1]",sizeof(BINDING));
    if (!b)
      return 0;
    b->uri = (XML_Char *)MALLOC("addBinding[2]",sizeof(XML_Char) * (len + EXPAND_SPARE));
    if (!b->uri) {
      FREE("addBinding",b);
      return 0;
    }
...
handleUnknownEncoding(XML_Parser parser, const XML_Char *encodingName)
{
  if (unknownEncodingHandler) {
    ...
    if (unknownEncodingHandler(unknownEncodingHandlerData, encodingName, &info)) {
      ENCODING *enc;
      unknownEncodingMem = MALLOC("handleUnknownEncoding[1]",XmlSizeOfUnknownEncoding());
	 ...
	 const char **nextPtr)
{
  ...
  for (;;) {
	...
	*eventEndPP = s;
	...
	if (groupSize)
	  groupConnector = (char *)REALLOC("doProlog[1]",groupConnector, groupSize *= 2);
	else
	  groupConnector = (char *)MALLOC("doProlog[1]",groupSize = 32);
	if (!groupConnector)
	  return XML_ERROR_NO_MEMORY;
      }
...
defineAttribute(ELEMENT_TYPE *type, ATTRIBUTE_ID *attId, int isCdata, const XML_Char *value)
{
  ...
  if (type->nDefaultAtts == type->allocDefaultAtts) {
    if (type->allocDefaultAtts == 0) {
      ...
      type->defaultAtts = (DEFAULT_ATTRIBUTE *)MALLOC("defineAttribute[1]",type->allocDefaultAtts*sizeof(DEFAULT_ATTRIBUTE));
    }
    else {
      type->allocDefaultAtts *= 2;
      type->defaultAtts = (DEFAULT_ATTRIBUTE *)REALLOC("defineAttribute[1]",type->defaultAtts,
...
static int dtdCopy(DTD *newDtd, const DTD *oldDtd)
{
  ...
  for (;;) {
    ...
    if (oldE->nDefaultAtts) {
      newE->defaultAtts = (DEFAULT_ATTRIBUTE *)MALLOC("dtdCopy[1]",oldE->nDefaultAtts * sizeof(DEFAULT_ATTRIBUTE));
      if (!newE->defaultAtts)
	return 0;
    }
...
int poolGrow(STRING_POOL *pool)
{
  ...
  if (pool->blocks && pool->start == pool->blocks->s) {
    ...
    pool->blocks = (BLOCK *)REALLOC("poolGrow[1]",pool->blocks, offsetof(BLOCK, s) + blockSize * sizeof(XML_Char));
    if (!pool->blocks)
      return 0;
    ...
    if (blockSize < INIT_BLOCK_SIZE)
      ...
      blockSize *= 2;
    tem = (BLOCK *)MALLOC("poolGrow[1]",offsetof(BLOCK, s) + blockSize * sizeof(XML_Char));
    if (!tem)
      return 0;
    tem->size = blockSize;
