...
BOOLEAN btk_isr(IN PKINTERRUPT  pInterrupt, IN void *pServiceContext);
    ...
    if (eint_pending) {
        ...
        */
        ...
        if (IS_SET(unit_p->bt_status, BT_ER_INTR)) {
            vector = bt_user_isr(unit_p, &unit_p->qh_err_fn, BT_IRQ_ERROR, 0);
        }
#else /* !defined(BT946) */
       vector = btp_user_isr(unit_p, &unit_p->qh_iack_fn, BT_IRQ_ERROR, 0, 0);
#endif /* !defined(BT946) */
