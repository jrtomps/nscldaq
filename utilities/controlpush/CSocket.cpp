...
{ 
  ...
  struct hostent* pEntry;
    ...
    CApplicationSerializer::Lock(); // <-- Critical region
...

{
  ...
  if(connect(m_Fd, (const sockaddr*)&Peer, sizeof(sockaddr_in)) < 0) {
    int sErrno = errno;
    string Ip, Svc;
    CApplicationSerializer::Lock(); //<-- Critical section start
    {
      char port[50];
    ...
    CApplicationSerializer::Unlock(); //<-- End Crit. Section.
...
CSocket::Service(const string& rService)
{
  ...
  if(sscanf(rService.c_str(), "%d", &nport) != 1) { // Non numeric service number.
    ...
    CApplicationSerializer::Unlock(); // <-- End critical rgn.
...
CSocket::OpenSocket()
{
  ...
  CApplicationSerializer::Lock(); //--> Critical region since
