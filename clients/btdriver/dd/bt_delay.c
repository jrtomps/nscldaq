...
extern int bt_get_sys_clk_rate(void); /* proto-type for VxWorks BSP */
...

{
    ...
    dns = usec * bt_get_sys_clk_rate() / 1000000; 
    ...
    int usec)
{
    ...
    */
...

    {
      ...
      /* Use the VxWorks BSP indepented system clack rate call */
      vx_status = taskDelay(usec * bt_get_sys_clk_rate() / 1000000 );
    ...
    /*
    ** This one does busy wait the processor
    ** Need to change when we have time
    */
    udelay(usec);
...
#error Must convert tickers to usec for OSF
  ...
  
 {
    ...
    vx_status = wdStart(
                 ...
                 (int) (usec * bt_get_sys_clk_rate() / 1000000), 
