...
map<CSocket::SocketState, string> CSocket::m_StateNames;
...
/*!
  ...
  */
CSocket::CSocket (    int am_Fd,  CSocket::SocketState am_State  ) :
  m_Fd(am_Fd),
  m_State(am_State)
...

{
  ...
  CSocket::SocketState expected = Disconnected;
...
\param oobState  This can be (Defaults to TRUE):
- TRUE Enables out of band inline data.
- FALSE disables out of band inline data.

*/
void 
CSocket::OOBInline(bool oobState)  
{ 
  int state((int)oobState);
  if(setsockopt(m_Fd, SOL_SOCKET, 
...
/*!
  Purpose:
     ...
     is called to get a dotted string.  Note that this entire function
     must run syncrhonized to the global application mutex since the (3)
     network database calls are assumed to be nonthreadsafe.
 ...
 */
string
CSocket::StateName(CSocket::SocketState state)
{
  StockStateMap();
