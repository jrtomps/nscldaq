...
class RunVariableBufferTests : public CppUnit::TestFixture {
private:

  // Test Suite and test registration.

  CPPUNIT_TEST_SUITE(RunVariableBufferTests);
  TEST(testConstruction);
  CPPUNIT_TEST_SUITE_END();

  // data:

  CRunVariableBuffer* m_pBuffer;


public:
  void setUp()
  {
    m_pBuffer = new CRunVariableBuffer; // Default size is good enough
  }
  void tearDown()
  {
    delete m_pBuffer;
...
CPPUNIT_TEST_SUITE_REGISTRATION(RunVariableBufferTests);
...
void
RunVariableBufferTests::testConstruction()
{
  ...
  AssertEQMsg("Buffer type mismatch: ", RUNVARBF, hdr.type);
