...
class DocumentationBufferTests : public CppUnit::TestFixture {
private:

  // Test Suite and test registration.

  CPPUNIT_TEST_SUITE(DocumentationBufferTests);
  TEST(testConstruction);
  CPPUNIT_TEST_SUITE_END();

  // data:

  CNSCLDocumentationBuffer* m_pBuffer;


public:
  void setUp()
  {
    m_pBuffer = new CNSCLDocumentationBuffer; // Default size is good enough
  }
  void tearDown()
  {
    delete m_pBuffer;
...
CPPUNIT_TEST_SUITE_REGISTRATION(DocumentationBufferTests);
...
void
DocumentationBufferTests::testConstruction()
{
  ...
  AssertEQMsg("Buffer type mismatch: ", PKTDOCBF, hdr.type);
