...
class PauseBufferTests : public CppUnit::TestFixture {
private:

  // Test Suite and test registration.

  CPPUNIT_TEST_SUITE(PauseBufferTests);
  TEST(testConstruction);
  CPPUNIT_TEST_SUITE_END();

  CNSCLPauseBuffer* m_pBuffer;


public:
  void setUp()
  {
    m_pBuffer = new CNSCLPauseBuffer; // Default size is good enough
  }
  void tearDown()
  {
    delete m_pBuffer;
...
CPPUNIT_TEST_SUITE_REGISTRATION(PauseBufferTests);
...
//
void
PauseBufferTests::testConstruction()
{
  ...
  AssertEQMsg("Buffer type field", PAUSEBF, bf.header.type);
