...
**                  SBS Bit 3 API to do send a programmed interrupt to the remote
**                  bus.
**
**      $Revision$
**
...
#error This example can only be compiled for Nanobus or NanoBridge.
...

    ...
    */
    ...
    /* 
    ** Send a programmed interrupt.
    */
    status = bt_send_irq(btd);
    if (status != BT_SUCCESS) {
        bt_perror(btd, status, name_p);
        /* don't exit here- we still need to close the device */
        main_ret = EXIT_FAILURE;
    } else {
        puts ("Success!  Programmed interrupt sent to the remote bus.");
    }
...
static void usage(const char * name_p) {
    printf ("%s is an example program demonstrating the use of the\n",
            name_p);
    puts   ("bt_send_irq() function of the Nanobus extension to the SBS Bit 3 C");
    puts   ("API to send a programmed interrupt to the remote bus.");
