...
#ifndef BINARYONRAMP_VERSION_H
#include <dshusage/binaryonramp_version.h>
#endif

#ifndef BINARYONRAMP_HELP_H
#include <dshusage/binaryonramp_help.h>
#endif

using namespace daqhwyapi;

/**
* @var binaryonramp_rtexception
* @brief Exception to throw for general runtime errors.
*
* Exception to throw for for general runtime errors.
*/
static RuntimeException binaryonramp_rtexception;
...
* @version 1.0.0
*/
class BinaryOnRamp: public Main {
  ...
  */
  private: void startProgram(PipeInjector& injector,String& file,String& args) {
    ...
    if (::pipe(childout) < 0) {
      ...
      throw binaryonramp_rtexception.format(CSTR("BinaryOnRamp::startProgram() failed to create pipe msg=\"%s\" rc=%d"),strerror_r(eno,ebuf,DAQHWYAPI_STRERROR_SIZE),eno);
    } 
    ...
    if ((my_child_pid = ::fork()) < 0) {
      ...
      ::close(childerr[0]); 
      ::close(childerr[1]); 
      throw binaryonramp_rtexception.format(CSTR("BinaryOnRamp::startProgram() failed to fork child process pipe msg=\"%s\" rc=%d"),strerror_r(eno,ebuf,DAQHWYAPI_STRERROR_SIZE),eno);
    } else {
      if (my_child_pid > 0) { // Parent process
        ...
        // And a BinaryRecordReader
        FdInputStream fchildout(childout[0]);
        BinaryRecordReader binreader(fchildout);
        binreader.setTyped(my_typed);
        injector.connect(binreader);
        ...
        if (::dup2(childout[1],1) < 0) {
          int eno = errno;
          char ebuf[DAQHWYAPI_STRERROR_SIZE+1];
          throw binaryonramp_rtexception.format(CSTR("BinaryOnRamp::startProgram() failed to dup2 child's stdout msg=\"%s\" rc=%d"),strerror_r(eno,ebuf,DAQHWYAPI_STRERROR_SIZE),eno);
        }
        ...
        if (::dup2(childerr[1],2) < 0) {
          int eno = errno;
          char ebuf[DAQHWYAPI_STRERROR_SIZE+1];
          throw binaryonramp_rtexception.format(CSTR("BinaryOnRamp::startProgram() failed to dup2 child's stderr msg=\"%s\" rc=%d"),strerror_r(eno,ebuf,DAQHWYAPI_STRERROR_SIZE),eno);
        }
  ...
  */
  private: void write_record(FdOutputStream& aStream,record_header_t& aHdr,ByteArray *data) {
    ...
    if (rc < Record::encode_buffer_size) {
      throw binaryonramp_rtexception.format(CSTR("BinaryOnRamp::write_record() failed to write record header."));
    }
    ...
    if (data != NULL) {
      if (data->length > 0) {
        ...
        if (rc < data->length) {
          throw binaryonramp_rtexception.format(CSTR("BinaryOnRamp::write_record() failed to write record data."));
        }
      }
    }
  }
  ...
  *
  * Run the main loop. The binaryonramp main loop reads records
    ...
    while (!finread.eof()) {
      ...
      if (recsiz > 0) { // Write header and data...
        if ((!have_beginrun)&&(rechdr.record_type != Record::type_beginrun)) {
          ...
          throw binaryonramp_rtexception.format(CSTR("BinaryOnRamp::mainloop() First record read from stdin is not a BEGINRUN record.  Record has type of %d \"%s\""),rechdr.record_type,thetyp.c_str());
