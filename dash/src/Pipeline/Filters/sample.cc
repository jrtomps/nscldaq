...
#include <dshusage/sample_version.h>
#endif

#ifndef SAMPLE_HELP_H
#include <dshusage/sample_help.h>
#endif

using namespace daqhwyapi;

/**
* @var sample_rtexception
* @brief Exception to throw for general runtime errors.
*
* Exception to throw for for general runtime errors.
*/
static RuntimeException sample_rtexception;
...
class Sample: public Main {
  ...
  */
  private: bool parseCommandline(int argc,char *argv[]) {
    ...
    if (!my_hasrange) {
      cerr << "sample requires at least one packet argument." << endl;
      return false;
    }
  ...
  */
  private: void write_record(FdOutputStream& aStream,record_header_t& aHdr,ByteArray *data) {
    ...
    if (rc < Record::encode_buffer_size) {
      throw sample_rtexception.format(CSTR("Sample::write_record() failed to write record header."));
    }
    ...
    if (data != NULL) {
      if (data->length > 0) {
        ...
        if (rc < data->length) {
          throw sample_rtexception.format(CSTR("Sample::write_record() failed to write record data."));
        }
      }
    }
  }
  ...
  *
  * Run the main loop. The sample main loop reads records
  ...
  */
  private: void mainloop() {
    ...
    // Set up a selector for stdout
    FdSelector selector;
    int outfd = foutput.getFD();
    selector.addWriteFd(outfd);
    ...
    while (!finread.eof()) {
      ...
      if (recsiz > 0) { // Write header and data...
        // If the record is in range then check if the write would block.
        // If it would block do not write the record.  
        if (in_range(rechdr)) {
          if (selector.poll() > 0) {
            if (selector.isWritable(outfd)) {
              write_record(foutput,rechdr,&recdata);
              foutput.flush();
            } // else drop the record
          }
        } else {  // Write the record even if the write would block.
          write_record(foutput,rechdr,&recdata);
          foutput.flush();
        } 
      }
